...
fn main() -> anyhow::Result<()> {
    ...
    let mut wallet = match Wallet::from_sqlite(&mut conn)? {
        ...
        None => {
            ...
            let (desc_id, descriptor) = parse_descriptor(desc1);
            let mut keyring = KeyRing::new(network, (desc_id, descriptor));

            // We then add descriptors to this keyring
            for descriptor_string in [desc2, desc3, desc4, desc5, desc6, desc6] {
                let keychain_and_descriptor = parse_descriptor(descriptor_string);
                keyring.add_descriptor(keychain_and_descriptor.0, keychain_and_descriptor.1, false);
            }
...
fn parse_descriptor(s: &str) -> (DescriptorId, Descriptor<DescriptorPublicKey>) {
    let desc = Descriptor::parse_descriptor(&Secp256k1::new(), s)
        .expect("failed to parse descriptor")
        .0;
    (desc.descriptor_id(), desc)
}
